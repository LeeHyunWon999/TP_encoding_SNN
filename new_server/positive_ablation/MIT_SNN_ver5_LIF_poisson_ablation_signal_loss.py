...
class SNN_MLP(nn.Module):
    def __init__(self, num_classes, num_encoders, hidden_size, leak, threshold_value, bias_option):
        ...
        self.signal1 = None  # 첫 번째 Linear 출력
        self.signal2 = None  # 두 번째 Linear 출력
    ...
    def forward(self, x: torch.Tensor):
        # 첫 번째 Linear-LIF 블록
        linear1_out = self.hidden[0](x)  # block1의 Linear 출력
        self.signal1 = linear1_out  # 저장
        x = self.hidden[1](linear1_out)  # LIF 적용

        # 두 번째 Linear-LIF 블록
        linear2_out = self.layer[0](x)  # block2의 Linear 출력
        self.signal2 = linear2_out  # 저장
        x = self.layer[1](linear2_out)  # LIF 적용
        return x
...
def check_accuracy(loader, model):
    ...
    with  torch.no_grad():
        for x, y in loader:         ############### train쪽에서 코드 복붙 시 (data, targets) 가 (x, y) 로 바뀌는 것에 유의할 것!!!!!!!!###############
            ...
            # 음의 시그널 패널티 항도 loss에 추가하기!
            negative1 = model.signal1[model.signal1 < 0]
            negative2 = model.signal2[model.signal2 < 0]
            penalty1 = torch.sum(negative1 ** 2)
            penalty2 = torch.sum(negative2 ** 2)
            final_loss += (penalty1 + penalty2) * negative_penalty_alpha
